<?php

error_reporting( E_ALL );
    /* Last updated with phpFlickr 2.3.2
     *
     * Edit these variables to reflect the values you need. $default_redirect
     * and $permissions are only important if you are linking here instead of
     * using phpFlickr::auth() from another page or if you set the remember_uri
     * argument to false.
     */
	$api_key = "68e9a4df375a157a22a8d79e4e9a43bb";
    	$api_secret = "3b4e45208b0d7aaf";

    //$api_key                 = "[your api key]";
    //$api_secret              = "[your api secret]";
    $default_redirect        = "/cimmyt/CIMMYTSocialModule/flickr/auth.php";
    $permissions             = "write";
    $path_to_phpFlickr_class = "";

    ob_start();
    require_once($path_to_phpFlickr_class . "phpFlickr.php");
    //@unset($_SESSION['phpFlickr_auth_token']);

	$_SESSION['phpFlickr_auth_token']="";

	if ( isset($_SESSION['phpFlickr_auth_redirect']) && !empty($_SESSION['phpFlickr_auth_redirect']) ) {
		$redirect = $_SESSION['phpFlickr_auth_redirect'];
		unset($_SESSION['phpFlickr_auth_redirect']);
	}

    $f = new phpFlickr($api_key, $api_secret);

    if (empty($_GET['frob'])) {
        $f->auth($permissions, false);
    } else {
        $f->auth_getToken($_GET['frob']);
	}

    if (empty($redirect)) {
		//print_r($_SESSION);

		echo "To authorise the app, please copy this:".$_SESSION['phpFlickr_auth_token']['_content']." at the_key.txt of your flickr root installation folder!";

		$fp = fopen('./the_key.txt', 'w');
		fwrite($fp, $_SESSION['phpFlickr_auth_token']['_content']);
		fclose($fp);

		//header("Location: " . $default_redirect);
    } else {
		header("Location: " . $redirect);
    }

?>
